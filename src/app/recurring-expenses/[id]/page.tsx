...
export default function EditRecurringExpensePage() {
  ...
  const expenseDocRef = useMemoFirebase(() => {
    ...
    return doc(firestore, 'couples', COUPLE_ID, 'recurringExpenses', id as string);
  }, [firestore, id]);

  const { data: expenseData, isLoading: isLoadingExpense } = useDoc<RecurringExpense>(expenseDocRef);
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    toast({ title: 'Despesa recorrente atualizada!', description: `"${values.description}" foi salva.` });
  ...
  return (
    <main className="container mx-auto p-4 md:p-8">
      <Button variant="outline" onClick={() => router.push('/recurring-expenses')} className="mb-4">
        <ArrowLeft className="mr-2 h-4 w-4" />
        Voltar
      </Button>
      <Card>
        <CardHeader>
          <CardTitle className="text-3xl">Editar Despesa Recorrente</CardTitle>
          <CardDescription>Altere os detalhes do gasto recorrente e salve.</CardDescription>
        </CardHeader>
        <Form {...form}>
